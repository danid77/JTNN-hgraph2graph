...
class HierMPNDecoder(nn.Module):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, latent_size, depthT, depthG, dropout, attention=False):
        ...
        self.itensor = torch.LongTensor([]).to(device)
        ...
        self.E_order = torch.eye(MolGraph.MAX_POS).to(device)
    ...
    ###########################################################################################################   
    def get_assm_score(self, src_graph_vecs, batch_idx, assm_vecs):
        if src_graph_vecs.size(0) != batch_idx.max().item() + 1:
            print(f"src_graph_vecs size: {src_graph_vecs.size(0)}, batch_idx max value: {batch_idx.max().item() + 1}")
            raise RuntimeError("Sizes of src_graph_vecs and batch_idx do not match.")
    ...
    def enum_attach(self, hgraph, cands, icls, nth_child):
        cands = self.itensor.new_tensor(cands)
        icls = self.itensor.new_tensor(icls).unsqueeze(1).repeat(1, len(cands)).flatten()
        icls_vecs = self.E_assm(icls)
        
        nth_child = self.itensor.new_tensor([nth_child] * len(cands.view(-1)))
        order_vecs = self.E_order.index_select(0, nth_child)

        # Ensure icls_vecs and order_vecs have the same size in the concatenation dimension
        if icls_vecs.size(0) != order_vecs.size(0):
            print(f"icls_vecs size: {icls_vecs.size(0)}, order_vecs size: {order_vecs.size(0)}")
            raise RuntimeError("Sizes of icls_vecs and order_vecs do not match.")
                    ...
                    if len(inter_cands) == 0:
                        ...
                        for x in anchor_smiles:
                            ...
                            if not isinstance(x, tuple):
                                x = (x,)
                            try:
                                icls.append(self.vocab[(smiles, x[0])][1])
                            except KeyError:
                                print(f"KeyError: ({smiles}, {x}) not found in vocab.")
                                icls.append(None)  # Append None to handle it later

                        # Remove None values
                        icls = [cls for cls in icls if cls is not None]

                        if not icls:
                            print("Warning: All anchor_smiles are missing keys in vocab. Skipping this entry.")
                            continue
                        ...
                        # Adjust the batch_idx to match the size of src_graph_vecs
                        # batch_idx = batch_idx.new_tensor( [bid] * len(inter_cands) )
                        
                        max_idx = src_graph_vecs.size(0) - 1
                        batch_idx = batch_idx.new_tensor([min(bid, max_idx)] * len(inter_cands))
                        ...
                        ###########################################################################################################
                        # Ensure batch_idx values are within the range of src_graph_vecs
                        if batch_idx.max().item() >= src_graph_vecs.size(0):
                            raise RuntimeError("batch_idx values are out of range of src_graph_vecs.")
                        ###########################################################################################################
