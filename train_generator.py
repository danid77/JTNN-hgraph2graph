...
parser.add_argument('--train', default='/home/lnptest/SB_jin/git_test/hgraph2graph_wandb/train_processed/')
...
parser.add_argument('--hidden_size', type=int, default=512) # 250
parser.add_argument('--embed_size', type=int, default=512) # 250
...
parser.add_argument('--print_iter', type=int, default=512) # 배치사이즈와 동일하게 
parser.add_argument('--save_iter', type=int, default=512) # 배치사이즈와 동일하게 
...
all_dataset = DataSampling(args.train, args.batch_size)
global_dataset = all_dataset.lend_data()
...
for epoch in range(args.epoch):
    dataset = random.sample(global_dataset, 5000)
    model.train()
    meters = torch.zeros(6).to(device)
    # beta = 0.0
    ...
    for batch in tqdm(dataset):
        ...
        if (best_loss > meters[1]) & (best_wacc < meters[2]):
            best_loss = meters[1]
            best_wacc = meters[2]
            # loss_list.append(total_step)
            ckpt = (model.state_dict(), optimizer.state_dict(), total_step, beta)
            torch.save(ckpt, os.path.join(args.save_dir, f"model_{total_step}.ckpt"))
